...
The [methods to construct a transaction](/reference/algokit-utils-ts/api/classes/types_composerdefault/#methods) are all named `add{TransactionType}` and return an instance of the composer so they can be chained together fluently to construct a transaction group.
