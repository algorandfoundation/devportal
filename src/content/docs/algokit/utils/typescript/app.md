...
The [`AppManager`](docs/code/classes/types_app_manager.AppManager) is a class that is used to manage app information.
...
- All app calls extend [`SendAppTransactionResult`](docs/code/modules/types_transaction#sendapptransactionresult), which has:
...
- Update and create calls extend [`SendAppUpdateTransactionResult`](docs/code/modules/types_transaction#sendappupdatetransactionresult), which has:
...
- Create calls extend [`SendAppCreateTransactionResult`](docs/code/modules/types_transaction#sendappcreatetransactionresult), which has:
...
The base type for specifying an app creation transaction is [`AppCreateParams`](docs/code/modules/types_composer#appcreateparams) (extended as [`AppCreateMethodCall`](docs/code/modules/types_composer#appcreatemethodcall) for ABI method call version), which has the following parameters in addition to the [common parameters](#common-app-parameters):
...
If you pass in `approvalProgram` or `clearStateProgram` as a string then it will automatically be compiled using Algod and the compilation result will be available via [`algorand.app.getCompilationResult`](docs/code/classes/types_app_manager.AppManager#getcompilationresult) (including the source map). To skip this behaviour you can pass in the compiled TEAL as `Uint8Array`.
...
The base type for specifying an app update transaction is [`AppUpdateParams`](docs/code/modules/types_composer#appupdateparams) (extended as [`AppUpdateMethodCall`](docs/code/modules/types_composer#appupdatemethodcall) for ABI method call version), which has the following parameters in addition to the [common parameters](#common-app-parameters):
...
The base type for specifying an app deletion transaction is [`AppDeleteParams`](docs/code/modules/types_composer#appdeleteparams) (extended as [`AppDeleteMethodCall`](docs/code/modules/types_composer#appdeletemethodcall) for ABI method call version), which has the following parameters in addition to the [common parameters](#common-app-parameters):
...
The base type for specifying an app call transaction is [`AppCallParams`](docs/code/modules/types_composer#appcallparams) (extended as [`AppCallMethodCall`](docs/code/modules/types_composer#appcallmethodcall) for ABI method call version), which has the following parameters in addition to the [common parameters](#common-app-parameters):
...
- [`algorand.app.getGlobalState(appId)`](docs/code/classes/types_app_manager.AppManager#getglobalstate) - Returns the current global state for the given app ID decoded into an object keyed by the UTF-8 representation of the state key with various parsed versions of the value (base64, UTF-8 and raw binary)
...
- [`AppManager.decodeAppState(state)`](docs/code/classes/types_app_manager.AppManager#decodeappstate) - Takes the raw response from the algod API for global state and returns a friendly generic object keyed by the UTF-8 value of the key
...
- [`algorand.app.getLocalState(appId, address)`](docs/code/classes/types_app_manager.AppManager#getlocalstate) - Returns the current local state for the given app ID and account address decoded into an object keyed by the UTF-8 representation of the state key with various parsed versions of the value (base64, UTF-8 and raw binary)
...
- [`algorand.app.getBoxNames(appId: bigint)`](docs/code/modules/#getboxnames) - Returns the current [box names](#boxname) for the given app ID
- [`algorand.app.getBoxValue(appId: bigint, boxName: BoxIdentifier)`](docs/code/modules/#getboxvalue) - Returns the binary value of the given box name for the given app ID
...
- [`algorand.app.getBoxValueFromABIType(request: {appId: bigint, boxName: BoxIdentifier, type: algosdk.ABIType}})`](docs/code/modules/#getboxvaluefromabitype) - Returns the parsed ABI value of the given box name for the given app ID for the provided ABI type
...
- [`AppManager.getBoxReference(boxId)`](docs/code/modules/#getboxreference) - Returns a `algosdk.BoxReference` representation of the given [box identifier / reference](#box-references), which is useful when constructing a raw `algosdk.Transaction`
...
- [`algorand.app.getById(appId)`](docs/code/classes/types_app_manager.AppManager#getbyid) - Returns current app information by app ID from an [`AppManager`](#appmanager) instance
- [`indexer.lookupAccountCreatedApplicationByAddress(indexer, address, getAll?, paginationLimit?)`](docs/code/modules/index.indexer#lookupaccountcreatedapplicationbyaddress) - Returns all apps created by a given account from [indexer](/algokit/utils/typescript/indexer/)

## Common app parameters

When interacting with apps (creating, updating, deleting, calling), there are some [`CommonAppCallParams`](docs/code/modules/types_composer#commonappcallparams) that you will be able to pass in to all calls in addition to the [common transaction parameters](/algokit/utils/typescript/algorand-client/#transaction-parameters):
...
When making an ABI call, the `args` parameter is replaced with a different type and there is also a `method` parameter per the [`AppMethodCall`](docs/code/modules/types_composer#appmethodcall) type:
