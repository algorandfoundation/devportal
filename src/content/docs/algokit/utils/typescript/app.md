...
- [`indexer.lookupAccountCreatedApplicationByAddress(indexer, address, getAll?, paginationLimit?)`](/docs/algokit-utils/typescript/latest/api/modules/index.indexer#lookupaccountcreatedapplicationbyaddress) - Returns all apps created by a given account from [indexer](/algokit/utils/typescript/indexer/)

## Common app parameters

When interacting with apps (creating, updating, deleting, calling), there are some [`CommonAppCallParams`](/docs/algokit-utils/typescript/latest/api/modules/types_composer#commonappcallparams) that you will be able to pass in to all calls in addition to the [common transaction parameters](/algokit/utils/typescript/algorand-client/#transaction-parameters):
