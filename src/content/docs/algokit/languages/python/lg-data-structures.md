...
As of Algorand Python 2.7, two new array types were introduced [`algopy.Array`](docs/_build/markdown/api-algopy#algopy.Array), a mutable array type
that supports statically sized native and ARC-4 elements and [`algopy.ImmutableArray`](docs/_build/markdown/api-algopy#algopy.ImmutableArray) that has
an immutable API and supports dynamically sized native and ARC-4 elements.

## Mutability vs Immutability

A value with an immutable type cannot be modified. Some examples are [`UInt64`](docs/_build/markdown/api-algopy#algopy.UInt64), [`Bytes`](docs/_build/markdown/api-algopy#algopy.Bytes), `tuple` and `typing.NamedTuple`.
...
- Requires [`.copy()`](docs/_build/markdown/api-algopy#algopy.Struct.copy) when mutable and creating additional references
...
- Requires [`.copy()`](docs/_build/markdown/api-algopy.arc4#algopy.arc4.Tuple.copy) when mutable and creating additional references
- Encoded as a single ARC-4 value on the stack
...
- Members are described by a field name and type
- Can be immutable if using the `frozen` class option and all members are also immutable
- Requires [`.copy()`](docs/_build/markdown/api-algopy.arc4#algopy.arc4.Struct.copy) when mutable and creating additional references
- Encoded as a single ARC-4 value on the stack
...
- Requires [`.copy()`](docs/_build/markdown/api-algopy#algopy.FixedArray.copy) if making additional references to the array or any mutable elements

### `algopy.Array`

- Can contain any type except transactions
- Dynamically sized, efficient for reading (when assembled off-chain). Inefficient to manipulate on-chain
- Requires [`.copy()`](docs/_build/markdown/api-algopy#algopy.Array.copy) if making additional references to the array or any mutable elements
...
- An immutable copy can be made for storage or returning from a contract by using the [`freeze`](docs/_build/markdown/api-algopy#algopy.ReferenceArray.freeze) method e.g.
...
- Requires [`.copy()`](docs/_build/markdown/api-algopy.arc4#algopy.arc4.DynamicArray) if making additional references to the array or mutable elements
...
- Use [`algopy.ReferenceArray.freeze`](docs/_build/markdown/api-algopy#algopy.ReferenceArray.freeze) to convert the array to an `algopy.ImmutableArray` for storage
