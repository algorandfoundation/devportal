...
next:
  link: /build/algokit/utils/ts/transaction/
sidebar:
  order: 6
---
...
:::note
The methods on this page require the [legacy AlgoKit Utils import method to access them](/build/algokit/utils/ts/overview#usage).
:::

## Transferring Algos

The key function to facilitate Algo transfers is `algokit.transferAlgos(transfer, algod)`, which returns a [`SendTransactionResult`](/build/algokit/utils/ts/transaction#sendtransactionresult) and takes a [`AlgoTransferParams`](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/docs/code/interfaces/types_transfer.AlgoTransferParams.md):

- All properties in [`SendTransactionParams`](/build/algokit/utils/ts/transaction#sendtransactionparams)
...
- `amount: AlgoAmount` - The [amount](/build/algokit/utils/ts/amount) of ALGOs to send
- `transactionParams?: SuggestedParams` - The optional [transaction parameters](/build/algokit/utils/ts/transaction#transaction-params)
- `note?: TransactionNote` - The [transaction note](/build/algokit/utils/ts/transaction#transaction-notes)
...
- `fundingSource?: SendTransactionFrom | TestNetDispenserApiClient` - The account that is the source of funds or a dispenser API client. If not specified, it will use the [dispenser](/build/algokit/utils/ts/account#dispenser)
...
The key function to facilitate asset transfers is `transferAsset(transfer, algod)`, which returns a [`SendTransactionResult`](/build/algokit/utils/ts/transaction#sendtransactionresult) and takes a [`TransferAssetParams`](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/docs/code/interfaces/types_transfer.TransferAssetParams.md):
