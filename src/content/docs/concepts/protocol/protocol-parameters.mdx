...
| Name                                                   | Value      | Variable        | Note                                                                                                                                                                     |
| ------------------------------------------------------ | ---------- | --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Max size of compiled TEAL code combined with arguments | 1000 bytes | LogicSigMaxSize | With an [additional fee](/concepts/transactions/fees#larger-transactions), the program can be up to 16000 bytes (MaxAbsoluteLogicSigProgramSize); arguments are unpriced |
...
| Name                                                               | Value             | Variable                 | Note                                                                                                                                                                                     |
| ------------------------------------------------------------------ | ----------------- | ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Current Max AVM/TEAL Version                                       | 13                | LogicSigVersion          | Available in consensus v42/AVM v13                                                                                                                                                       |
| Page size of compiled approval + clear TEAL code                   | 2048 bytes        | MaxAppProgramLen         | by default, each application has a single page                                                                                                                                           |
| Max extra app pages                                                | 3                 | MaxExtraAppProgramPages  | an application can "pay" for additional pages via minimum balance; up to 7 pages (MaxAbsoluteExtraProgramPages) for an [additional fee](/concepts/transactions/fees#larger-transactions) |
...
| Max combined size of arguments                                     | 2048 bytes        | MaxAppTotalArgLen        | Up to 16384 bytes (MaxAbsoluteTotalArgLen) for an [additional fee](/concepts/transactions/fees#larger-transactions)                                                                      |
...
:::note
In consensus v42 and AVM v13, the `txn.Access` list provides access to additional resources (up to 16) beyond the foreign references limited by `MaxAppTotalTxnReferences=8`. Foreign references are shared among accounts, ASAs, applications, and box storage, while the access list allows for more flexible and granular resource access.
:::
