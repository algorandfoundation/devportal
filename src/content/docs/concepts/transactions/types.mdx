...
An Application Update Transaction modifies an existing application's logic by providing new Approval and Clear programs, and from consensus v42 it can also change the application's extra program pages and global state schema. Only the current application's Approval Program can authorize this update.
...
- Neither `"apep"` nor `"apgs"` is present, so the application keeps the extra program pages and global state schema it already has

An update that carries `"apep"`, `"apgs"`, or both changes the application's size. The transaction below gives application 51 one extra program page and a global state schema of two integers and one byte slice:
...
Both fields are applied absolutely rather than as adjustments, so a size-changing update must state the value it wants for each of them. Omitting `"apep"` from the transaction above would set the application to zero extra program pages, and omitting `"apgs"` would empty its global state schema. The sender of a size-changing update takes on the minimum balance requirement for the extra pages and the global state. See [Size Changes on Update](/concepts/transactions/reference#size-changes-on-update) for the constraints these transactions must satisfy.
...
A Heartbeat Transaction allows validator nodes to signal they are operational, even when they haven't proposed blocks recently. These transactions are particularly important for accounts with smaller stakes that might not frequently propose blocks.
