...
Smart contracts can read the header fields of recent blocks directly in the Algorand Virtual Machine (AVM) using the `block` opcode, for blocks earlier than the transaction's first valid round. Available fields include the seed, timestamp, proposer, fees collected, the hash of the block's predecessor, and the block's transaction commitments in each supported hash function (SHA512_256, SHA256, and SHA512). This allows a smart contract to verify information about a previous block and the transactions it committed to. See the [TEAL Opcodes](/reference/algorand-teal/opcodes) reference for the full field list.
