...
TODO(joe-p): Python example

```ts
  // TODO(joe-p): determine the name
  @abimethod({ unsafeDisableValidation: true })  
  staticValue(staticBytes: StaticBytes<32>): uint64 {
    const array: [StaticBytes<32>, uint64] = [
      // If staticBytes is more than 40 bytes, it will overflow into the uint64
      staticBytes,
