...
:::note
ExtraProgramPages or Extra App Pages specifies the number of additional pages for each of the application's approval and clear state programs. An ExtraProgramPages of 1 means 2048 more total bytes, or 1024 for each program. This effectively allows the size limit to be increased from 2KB to 8KB. This parameter allows up to 3 additional pages to be requested, or up to 7 for an [additional fee](/concepts/transactions/fees#larger-transactions). However, requesting additional pages will increase the minimum balance requirement for creating the application.
:::
