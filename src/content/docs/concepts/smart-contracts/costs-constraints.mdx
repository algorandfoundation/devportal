...
| Type             | Constraint                                                                                                                                                                                                                                                        |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Logic Signatures | Standard max size: 1000 bytes for logic signatures (consensus parameter LogicSigMaxSize). Components: The bytecode plus the length of all arguments. With an [additional fee](/concepts/transactions/fees#larger-transactions), program bytes can be up to 16,000 |
| Smart Contracts  | Max size: 2048\*(1+ExtraProgramPages) bytes Components: ApprovalProgram + ClearStateProgram                                                                                                                                                                       |

:::note
ExtraProgramPages or Extra App Pages specifies the number of additional pages for each of the application's approval and clear state programs. An ExtraProgramPages of 1 means 2048 more total bytes, or 1024 for each program. This effectively allows the size limit to be increased from 2KB to 8KB. This parameter allows up to 3 additional pages to be requested, or up to 7 for an [additional fee](/concepts/transactions/fees#larger-transactions). Requesting additional pages increases the minimum balance requirement of the account that requests them, by 100,000 microAlgos per page. At creation that is the creator's account. From consensus v42, an update transaction can also change the page count, and the account that sends such an update takes on the requirement for the pages in place of the account that held it before; see [Size Changes on Update](/concepts/transactions/reference#size-changes-on-update).
:::
...
| Combined Size of Arguments     | The maximum combined size of arguments is 2048 bytes. This limit is defined by the consensus parameter `MaxAppTotalArgLen`. Up to 16,384 bytes are possible for an [additional fee](/concepts/transactions/fees#larger-transactions)                                                                                                                               |
| Max Size of Compiled TEAL Code | The standard maximum size of compiled TEAL code combined with arguments is 1000 bytes. This limit is defined by the consensus parameter `LogicSigMaxSize`. With an [additional fee](/concepts/transactions/fees#larger-transactions), the program can be up to 16,000 bytes                                                                                        |
...
| Box Storage          | 1 to 64 bytes  | Up to 32KB (32,768 bytes) | Yes                    | Key does not contribute to box size and values > 2,048 bytes need additional references | Boxes persist after app deletion but lock the minimum balance if not deleted beforehand    |
