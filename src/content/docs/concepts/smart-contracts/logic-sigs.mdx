...
:::note
[Post-quantum accounts](/concepts/accounts/post-quantum) can also delegate signature authority, but the signed payload differs from Ed25519 delegation (`"PQProgram" || authorizer address || program` rather than `"Program" || program`), so the scheme-appropriate signing path must be used, for example `algokey pq sign-program`. Transactions authorized by a Falcon-delegated logic signature pay the post-quantum minimum fee of 0.003 Algo. See [Post-Quantum Accounts](/concepts/accounts/post-quantum#delegated-logic-signatures) for details, including a caveat about delegations and rekeyed accounts.
:::
...
- **_Potential for Malicious Use_**: Most wallets do not support signing delegated logic signatures, as this operation is potentially dangerous. A malicious delegated logic signature can remain dormant for years and can allow to siphon out funds from an account much later. Note also that a delegation follows the authorized address: a logic signature delegated to address `B` is honored for _any_ account whose authorized address is `B`, including accounts later rekeyed to it. If a delegated program must only ever spend from one specific account, the program itself must check the sender address.
...
- **_Size and Cost Constraints_**: The standard maximum size of compiled TEAL code combined with arguments is 1000 bytes (consensus parameter LogicSigMaxSize), and the maximum cost of TEAL code is 20000. With an [additional fee](/concepts/transactions/fees#larger-transactions), the program can be up to 16,000 bytes. Arguments are never charged because they can be modified after signing, so the required fee cannot depend on them. Up to 1000 bytes of arguments are allowed per logic signature without group size pooling.
