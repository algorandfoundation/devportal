...
- **_Size and Cost Constraints_**: The maximum size of compiled TEAL code combined with arguments is 1000 bytes, and the maximum cost of TEAL code is 20000. Programs up to 16,000 bytes are possible for an [additional fee](/concepts/transactions/fees#larger-transactions). Arguments are never charged because they can be modified after signing, so the required fee cannot depend on them. Up to 1000 bytes of arguments are allowed per logic signature without group size pooling.
