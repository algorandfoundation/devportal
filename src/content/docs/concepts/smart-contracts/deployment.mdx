...
- TEAL Compilation: Smart contracts are written in [Algorand Python](/concepts/smart-contracts/languages/python) or [Algorand TypeScript](/concepts/smart-contracts/languages/typescript) and compiled to TEAL, which is assembled into the AVM bytecode submitted on-chain.
...
- Algokit utils py
  ...
  - explicit control over immutability (update/upgrade) and permanence (delete)
    - TMPL_UPDATABALE
    - TMPL_DELETABLE
  - id or name+deployer
