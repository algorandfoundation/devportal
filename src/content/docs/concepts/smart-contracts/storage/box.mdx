...
App a is the  app that can read the contents of its boxes on-chain. Since AVM v13, app a can permit any application to read its boxes on-chain by setting the `AppForeignBoxReads` parameter; see [Family Box Access](#family-box-access). Recall that anybody can read everything from off-chain using the algod or indexer APIs.
