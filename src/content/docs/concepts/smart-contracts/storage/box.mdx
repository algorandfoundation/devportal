...
A box can be read or written on-chain by the application that owns it. Starting with AVM v13 (consensus v42), an owning application can relax that restriction for its own boxes by setting one of two parameters with the `app_params_set` opcode:

| Parameter            | Effect                                                                                                                                       |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `AppForeignBoxReads` | Any application may read the owning application's boxes on-chain.                                                                            |
| `AppFamilyBoxAccess` | Applications sharing the owning application's creator address may read and write its boxes, including creating, resizing, and deleting them. |
...
:::caution
`AppFamilyBoxAccess` grants other applications write access to your application's stored data, lets them delete your boxes, and lets them change your application's minimum balance requirement. Because family membership is evaluated at access time, the grant also covers applications the creator has not deployed yet. Applications in a family share mutable state and must be designed, reviewed, and upgraded together: updating one family member can affect the data and balance of another. Enable it only when you specifically need to distribute logic across multiple applications, such as when working around the program size limit.
:::

### Re-entrancy and family-shared boxes
...
Family-shared boxes break that guarantee, because a _different_ application can now modify a box the suspended application depends on:

```
A writes its box "b"                  A now relies on b == "AA"
  A → X            foreign app (different creator)
      X → C        family member of A (same creator)
          C writes A's box "b" → "CC"
A resumes, still assuming b == "AA"   ← silent corruption
```

The existing re-entrancy rule does not catch this, because `C` is not `A`; the application was never re-entered. AVM v13 therefore adds a second, box-scoped check:

> An application cannot write a family-shared box when a foreign application sits on the call stack between it and a family application that has already read or written a family-shared box; the write fails.
...
| `A` writes `b`, then `A → C` directly, `C` writes `b`         | Allowed | No foreign application separates them                                             |
...
| --------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| The check fires at the write, not at the call | A foreign application can still call into a family member for read-only queries, so patterns like price oracles and registries keep working               |
...
Boxes shared only through `AppForeignBoxReads` are never subject to this check, because read-only access has nothing to clobber.
...
App a can read the contents of its boxes on-chain. Since AVM v13, app a can permit any application to read its boxes on-chain by setting the `AppForeignBoxReads` parameter; see [Family Box Access](#family-box-access). Recall that anybody can read everything from off-chain using the algod or indexer APIs.
...
Since AVM v13, each box opcode has an `app_box_*` counterpart that performs the same operation on a box owned by another application, taking an additional argument for that application's ID: `app_box_create`, `app_box_del`, `app_box_extract`, `app_box_get`, `app_box_len`, `app_box_put`, `app_box_replace`, `app_box_resize`, and `app_box_splice`. These require the owning application to have permitted the access, as described in [Family Box Access](#family-box-access). For the authoritative opcode signatures, see the [Opcodes List](/reference/algorand-teal/opcodes).
