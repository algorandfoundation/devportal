...
The two mechanisms grant these levels of access:
...
App a can read the contents of its boxes on-chain. Since AVM v13, app a can permit any application to read its boxes on-chain by setting the `AppForeignBoxReads` parameter; see [Foreign box reads](#foreign-box-reads). Recall that anybody can read everything from off-chain using the algod or indexer APIs.
