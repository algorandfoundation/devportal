...
The creator's account must hold a balance covering this MBR. An update transaction that changes the extra program pages or the global state schema makes its sender the application's size sponsor. The sponsor's account is the one whose MBR carries the terms for the extra program pages and the global state schema, and the previous sponsor's MBR is reduced by the terms it had been carrying. The 100,000 microAlgos for the application itself stays with the creator's account in every case, and a size-changing update sent by the creator leaves the creator as the sponsor. See [Size Changes on Update](/concepts/transactions/reference#size-changes-on-update) for the rules those transactions follow.
...
- Boxes can be created by an application and accessed on-chain by the application that created it. Since AVM v13, an application can opt in to letting other applications read its boxes, or letting applications with the same creator read and write them; see [Sharing Boxes Between Applications](/concepts/smart-contracts/storage/box/#sharing-boxes-between-applications). Box contents are readable by anyone off-chain in all cases. Keep in mind that the Minimum Balance Requirement (MBR) of the application will be increased depending on its size. This means that a contract that intends to use boxes, must be funded beforehand.
