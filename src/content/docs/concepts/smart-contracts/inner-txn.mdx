...
- An application that shares its boxes with same-creator applications is subject to an extra restriction since AVM v13. Once such an application has read or written a shared box, another application in the same family cannot write that box while an application with a different creator sits between them on the call stack; the write fails. See [Re-entrancy and family-shared boxes](/concepts/smart-contracts/storage/box#re-entrancy-and-family-shared-boxes).

:::note
Since AVM v13, reading or writing another application's boxes does not require an inner transaction at all. The `app_box_*` opcodes act on the box directly, provided the owning application has permitted the access. See [Family Box Access](/concepts/smart-contracts/storage/box#family-box-access).
:::
