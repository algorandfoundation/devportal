...
- An application that shares its boxes with same-creator applications is subject to an extra restriction since AVM v13. Once such an application has read or written a shared box, another application in the same family cannot write that box while an application with a different creator sits between them on the call stack; the write fails. See [Re-entrancy in family box access](/concepts/smart-contracts/storage/box#re-entrancy-in-family-box-access).
