...
As with all assets, the mutable addresses can be changed using contract code similar to the code below. Note these addresses cannot be changed once set to an empty value.
...
- An application that shares its boxes with same-creator applications is subject to an extra restriction since AVM v13. Once such an application has read or written a shared box, another application in the same family cannot write a family-shared box while an application with a different creator sits between them on the call stack; the write fails. See [Re-entrancy in family box access](/concepts/smart-contracts/storage/box#re-entrancy-in-family-box-access).

:::note
Since AVM v13, reading or writing another application's boxes does not require an inner transaction at all. The `app_box_*` opcodes act on the box directly, provided the owning application has permitted the access. See [Accessing another application's boxes](/concepts/smart-contracts/storage/box#accessing-another-applications-boxes).
:::
