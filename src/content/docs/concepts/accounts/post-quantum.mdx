...
Algorand supports **post-quantum accounts** secured by **Falcon-1024** signatures, a lattice-based signature scheme designed to resist attacks by quantum computers. Post-quantum accounts work alongside existing Ed25519 accounts. They hold Algos and assets, send transactions, and interact with applications like any other account, but their transactions are authorized by a quantum-resistant signature instead of an elliptic-curve one.
...
Every existing Algorand account is controlled by an Ed25519 key pair. Ed25519 is an elliptic-curve scheme, and elliptic-curve cryptography is breakable by a sufficiently large quantum computer running Shor's algorithm. No such computer is known to exist today, but assets secured on a blockchain need to remain secure for decades, and an attacker can record public keys now and attack them later.
