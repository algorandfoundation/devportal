...
Rekeying is a powerful protocol feature that enables an Algorand account holder to maintain a static public address while dynamically rotating the authoritative private spending key(s). This is accomplished by issuing a transaction with the `rekey-to` field set the authorized address field within the account object. Future transaction authorization using the account's public address must be provided by the spending key(s) associated with the authorized address, which may be a single key address, multisignature address, logic signature program address, or [post-quantum address](/concepts/accounts/post-quantum). Rekeying an account only affects the authorizing address for that account. An account is distinct from an address, so several essential points may not be obvious:
...
A confirmed `rekey-to` transaction defines, modifies, or removes the `auth-addr` field of the account object. Defining or modifying means only the corresponding authorized address's private spending key(s) may authorize future transactions for this public address. Rekeying an account back to its own address removes the `auth-addr` field entirely — the protocol never stores an `auth-addr` equal to the account’s own address.

To provide maximum flexibility in key management options, the `auth-addr` may be specified within a `rekey-to` transaction as a distinct foreign address representing a single key address, multisignature address, logic signature program address, or post-quantum address. The protocol does not validate control of the required spending key(s) associated with the authorized address defined by `--rekey-to` parameter when the `rekey-to` transaction is sent. This is by design and affords additional privacy features to the new authorized address. It is incumbent upon the user to ensure proper key management practices and `--rekey-to` assignments.
...
:::note
Rekeying is also how an existing account will eventually adopt **quantum-resistant signatures**: rekey to a [post-quantum address](/concepts/accounts/post-quantum) and the account keeps its public address while a Falcon-1024 key authorizes all future transactions. This is a forward-looking capability. Wallets and tooling may not support post-quantum signing yet, so see the migration caveats on the linked page before rekeying a real account.
:::

## Authorized Addresses

The balance record of every account includes the `auth-addr` field, which, when populated, defines the required authorized address to be evaluated during transaction validation. For a standard (non-rekeyed) account, the `auth-addr` field is empty — signing authority rests with the account’s own address and the private spending key created during account generation. The `auth-addr` field is only stored and displayed after the network confirms an authorized `rekey-to` transaction, and it is never stored equal to the account’s own address.
...
**There are two different AuthAddr fields**
The `auth-addr` described above belongs to the account’s balance record, and `goal account dump` displays it as `spend`. The ledger is what writes it: a confirmed `rekey-to` transaction sets or clears it, and closing the account removes it along with the rest of the record.

A signed transaction carries its own, separate authorization field, `AuthAddr`, encoded as [`sgnr`](/concepts/transactions/reference#authaddr). It names the address whose spending key authorized that one transaction, and the signer sets it rather than the ledger.

Populate `sgnr` only when the sender has been rekeyed. When an account authorizes its own transaction, omit `sgnr` entirely rather than setting it to the sender’s own address. As of consensus v42, the network rejects any transaction whose `sgnr` is present and equal to its `snd`. Consensus versions before v42 accepted that redundant value, so tooling that always populates `sgnr` needs to be updated to omit it for accounts that have not been rekeyed.
...
We will construct an unsigned transaction using `goal` with the `--out` flag to write the unsigned transaction to a file:
