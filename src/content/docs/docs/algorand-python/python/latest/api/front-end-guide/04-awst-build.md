...
class Visitor {
  ...
  visitNumericLiteralExpression(expr) {
    return new IntegerExpressionBuilder(
      new IntegerConstant({
        value: expr.value,
      }),
    );
...
The last step once you have built your AWST tree is to produce the two required artifacts discussed [earlier](/docs/algorand-python/python/latest/api/front-end-guide/01-calling-puya/) and then invoke puya.
