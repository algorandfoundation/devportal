...
[`algopy.Bytes`](/docs/algorand-python/python/latest/api/api-algopy/#algopy.Bytes) represents the underlying AVM `bytes[]` type. It is intended
...
See [Python builtins](/docs/algorand-python/python/latest/guides/lg-builtins/#len---length) for an explanation of why `len()` isn’t supported.

[See a full example](https://github.com/algorandfoundation/puya/blob/main/test_cases/stubs/bytes.py).

### String

[`String`](/docs/algorand-python/python/latest/api/api-algopy/#algopy.String) is a special Algorand Python type that represents a UTF-8 encoded string.
...
[`algopy.BigUInt`](/docs/algorand-python/python/latest/api/api-algopy/#algopy.BigUInt) represents a variable length (max 512-bit) unsigned integer stored
as `bytes[]` in the AVM.
...
[`Asset`](/docs/algorand-python/python/latest/api/api-algopy/#algopy.Asset) represents a logical Asset, backed by a `uint64` ID.
It has various asset related methods that can be called from the type.

### Application

[`Application`](/docs/algorand-python/python/latest/api/api-algopy/#algopy.Application) represents a logical Application, backed by a `uint64` ID.
...
The `int`, `str` and `bytes` built-in types are currently only supported as [module-level constants](/docs/algorand-python/python/latest/guides/lg-modules/) or literals.
