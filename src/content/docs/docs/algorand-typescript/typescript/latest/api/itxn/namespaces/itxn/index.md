---
editUrl: false
next: false
prev: false
title: 'itxn'
---

## Classes

- [ApplicationCallItxnParams](/algorand-typescript/api/itxn/namespaces/itxn/classes/applicationcallitxnparams/)
- [AssetConfigItxnParams](/algorand-typescript/api/itxn/namespaces/itxn/classes/assetconfigitxnparams/)
- [AssetFreezeItxnParams](/algorand-typescript/api/itxn/namespaces/itxn/classes/assetfreezeitxnparams/)
- [AssetTransferItxnParams](/algorand-typescript/api/itxn/namespaces/itxn/classes/assettransferitxnparams/)
- [KeyRegistrationItxnParams](/algorand-typescript/api/itxn/namespaces/itxn/classes/keyregistrationitxnparams/)
- [PaymentItxnParams](/algorand-typescript/api/itxn/namespaces/itxn/classes/paymentitxnparams/)

## Interfaces

- [ApplicationCallFields](/algorand-typescript/api/itxn/namespaces/itxn/interfaces/applicationcallfields/)
- [ApplicationCallInnerTxn](/algorand-typescript/api/itxn/namespaces/itxn/interfaces/applicationcallinnertxn/)
- [AssetConfigFields](/algorand-typescript/api/itxn/namespaces/itxn/interfaces/assetconfigfields/)
- [AssetConfigInnerTxn](/algorand-typescript/api/itxn/namespaces/itxn/interfaces/assetconfiginnertxn/)
- [AssetFreezeFields](/algorand-typescript/api/itxn/namespaces/itxn/interfaces/assetfreezefields/)
- [AssetFreezeInnerTxn](/algorand-typescript/api/itxn/namespaces/itxn/interfaces/assetfreezeinnertxn/)
- [AssetTransferFields](/algorand-typescript/api/itxn/namespaces/itxn/interfaces/assettransferfields/)
- [AssetTransferInnerTxn](/algorand-typescript/api/itxn/namespaces/itxn/interfaces/assettransferinnertxn/)
- [KeyRegistrationFields](/algorand-typescript/api/itxn/namespaces/itxn/interfaces/keyregistrationfields/)
- [KeyRegistrationInnerTxn](/algorand-typescript/api/itxn/namespaces/itxn/interfaces/keyregistrationinnertxn/)
- [PaymentFields](/algorand-typescript/api/itxn/namespaces/itxn/interfaces/paymentfields/)
- [PaymentInnerTxn](/algorand-typescript/api/itxn/namespaces/itxn/interfaces/paymentinnertxn/)

## Type Aliases

- [ItxnParams](/algorand-typescript/api/itxn/namespaces/itxn/type-aliases/itxnparams/)
- [TxnFor](/algorand-typescript/api/itxn/namespaces/itxn/type-aliases/txnfor/)

## Functions

- [applicationCall](/algorand-typescript/api/itxn/namespaces/itxn/functions/applicationcall/)
- [assetConfig](/algorand-typescript/api/itxn/namespaces/itxn/functions/assetconfig/)
- [assetFreeze](/algorand-typescript/api/itxn/namespaces/itxn/functions/assetfreeze/)
- [assetTransfer](/algorand-typescript/api/itxn/namespaces/itxn/functions/assettransfer/)
- [keyRegistration](/algorand-typescript/api/itxn/namespaces/itxn/functions/keyregistration/)
- [payment](/algorand-typescript/api/itxn/namespaces/itxn/functions/payment/)
- [submitGroup](/algorand-typescript/api/itxn/namespaces/itxn/functions/submitgroup/)
