...
A contract in Algorand TypeScript is defined by declaring a class that extends the `Contract` or `BaseContract` types exported by `@algorandfoundation/algorand-typescript`. See [ABI routing](/docs/algorand-typescript/typescript/latest/language-guide/reference/abi-routing/) docs for more on the differences between these two options.
