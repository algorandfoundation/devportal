...
- An `options` object, which has:
  - [Common transaction parameters](/docs/algokit-utils/python/latest/guides/algorand-client/#transaction-parameters) (not for TestNet Dispenser API)
  - [Execution parameters](/docs/algokit-utils/python/latest/guides/algorand-client/#sending-a-single-transaction) (not for TestNet Dispenser API)
...
All 3 variants return an `EnsureFundedResponse` (and the first two also return a [single transaction result](/docs/algokit-utils/python/latest/guides/algorand-client/#sending-a-single-transaction)) if a funding transaction was needed, or `None` if no transaction was required:
...
If you are using the TestNet Dispenser API then the `transaction_id` is useful if you want to use the [refund functionality](/docs/algokit-utils/python/latest/guides/dispenser-client/#registering-a-refund).
...
- Get a dispenser via [account manager](/docs/algokit-utils/python/latest/guides/account/#dispenser) - either automatically from [LocalNet](https://github.com/algorandfoundation/algokit-cli/blob/main/docs/features/localnet.md) or from the environment
- By programmatically creating one of the many account types via [account manager](/docs/algokit-utils/python/latest/guides/account/#accounts)
- By programmatically interacting with [KMD](/docs/algokit-utils/python/latest/guides/account/#kmd-account-management) if running against LocalNet
- By using the [AlgoKit TestNet Dispenser API client](/docs/algokit-utils/python/latest/guides/dispenser-client/) which can be used to fund accounts on TestNet via a dedicated API service