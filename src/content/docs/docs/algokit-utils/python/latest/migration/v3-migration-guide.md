...
The existing `ApplicationClient` (untyped app client) class is still present until at least v4, but it's worthwhile migrating to the new [`AppClient` and `AppFactory` classes](/docs/algokit-utils/python/latest/concepts/building/app-client/). These new clients are [ARC-56](https://github.com/algorandfoundation/ARCs/pull/258) compatible, but also support [ARC-32](https://github.com/algorandfoundation/ARCs/blob/main/ARCs/arc-0032.md) app specs and will continue to support this indefinitely until such time the community deems they are deprecated.
...
- The semantics to resolve the application is now available via [simpler entrypoints within `algorand.client`](/docs/algokit-utils/python/latest/concepts/building/app-client/#appclient)
