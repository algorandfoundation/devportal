...
description: 'This example demonstrates how to query application boxes using the AlgodClient methods: application_boxes() and application_box_by_name()'
sidebar:
  order: 11
---
...
- application*box_by_name(app_id, box_name_bytes) returns Box with .name, .value, .round* attributes
- BoxDescriptor.name is bytes (not base64)
- Box.name and Box.value are bytes (not base64)
- Box.round\_ uses underscore to avoid Python reserved keyword
  Prerequisites:
...
```bash
cd examples
uv run python algod_client/11_application_boxes.py
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-py/blob/main/examples/algod_client/11_application_boxes.py)

```python title="11_application_boxes.py"
...
- **Application Boxes**
- [TEAL Compile and Disassemble](/docs/algokit-utils/python/latest/examples/algod-client/12-teal-compile/)
