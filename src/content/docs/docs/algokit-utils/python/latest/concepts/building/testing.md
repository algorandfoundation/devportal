...
The `algorand` fixture provides access to an [`AlgorandClient`](/docs/algokit-utils/python/latest/concepts/core/algorand-client/) instance which exposes the following properties commonly used in testing:
...
- `algorand.account` - [`AccountManager`](/docs/algokit-utils/python/latest/concepts/core/account/) for creating and managing test accounts
...
The key when generating a test account is getting hold of a [dispenser](/docs/algokit-utils/python/latest/concepts/building/testing/transfer/#dispenser) and then [ensuring the test account is funded](/docs/algokit-utils/python/latest/concepts/building/testing/transfer/#ensure_funded).
...
- `algorand.account.localnet_dispenser()` - Gets the LocalNet [dispenser](/docs/algokit-utils/python/latest/concepts/building/testing/transfer/#dispenser) account for funding
- `algorand.account.dispenser_from_environment()` - Gets dispenser from environment variables or LocalNet
- `algorand.account.ensure_funded(account, dispenser, min_spending_balance=...)` - [Ensures the account is funded](/docs/algokit-utils/python/latest/concepts/building/testing/transfer/#ensure_funded) with a minimum balance
...
When testing functionality that involves [Algorand Standard Assets (ASAs)](/docs/algokit-utils/python/latest/concepts/building/testing/asset/), you can create test assets using a pytest fixture or helper function. This pairs with a funded test account fixture to create ephemeral assets for each test or test suite.
...
When testing [asset transfers](/docs/algokit-utils/python/latest/concepts/building/testing/asset/), the receiver must first opt in to the asset before receiving it. You can then transfer assets and assert on the resulting balances.
...
When testing [smart contract deployments](/docs/algokit-utils/python/latest/concepts/building/testing/app-deploy/), you can use the [`AppFactory`](/docs/algokit-utils/python/latest/concepts/building/testing/app-client/#appfactory) to deploy an application and then assert on the deployment result. The deploy result includes the operation performed, the app ID, and the app address.
...
When testing [application calls](/docs/algokit-utils/python/latest/concepts/building/testing/app-client/), you can use an [`AppClient`](/docs/algokit-utils/python/latest/concepts/building/testing/app-client/#appclient) to call ABI methods and assert on the return values. The `app_client.send.call()` method returns a result with an `abi_return` field containing the decoded ABI return value.
...
When testing [box storage](/docs/algokit-utils/python/latest/concepts/building/testing/app-client/#boxes) operations, you need to fund the application account to cover the minimum balance requirement (MBR) for boxes, then create, write, and read boxes via the [`AppClient`](/docs/algokit-utils/python/latest/concepts/building/testing/app-client/#appclient). Box references must be included in the transaction so the AVM can access them.
