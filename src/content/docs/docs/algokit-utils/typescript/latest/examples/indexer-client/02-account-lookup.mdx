...
description: 'This example demonstrates how to lookup account information using the IndexerClient lookupAccountById() and searchForAccounts() methods.'
sidebar:
  order: 2
---
...
This example demonstrates how to lookup account information using the IndexerClient lookupAccountById() and searchForAccounts() methods.
...
```bash
cd examples
npm run example indexer_client/02-account-lookup.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/indexer_client/02-account-lookup.ts)

```typescript title="02-account-lookup.ts"
/**
...
async function main() {
  ...
  try {
    ...
    printInfo(
      `  - amountWithoutPendingRewards: ${formatMicroAlgo(account.amountWithoutPendingRewards)}`,
    );
  ...
  try {
    ...
    if (searchResult.accounts.length > 0) {
      ...
      for (const account of searchResult.accounts) {
        printInfo(
          `  - ${shortenAddress(account.address)}: ${formatMicroAlgo(account.amount)} (status: ${account.status})`,
        );
      }
    }
  ...
  try {
    ...
    printError(
      `Online account search failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
