...
description: 'This example demonstrates how to configure transaction signers on AlgorandClient:'
sidebar:
  order: 3
---
...
[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/algorand_client/03-signer-config.ts)

```typescript title="03-signer-config.ts"
/**
...
async function main() {
  ...
  printInfo(
    'setSignerFromAccount() registers the signer from an account that can sign transactions',
  );
  ...
  printInfo(
    'setDefaultSigner() sets a signer that will be used when no specific signer is registered',
  );
  ...
  printInfo(
    'algorand.account.getSigner(address) retrieves the TransactionSigner registered for an address',
  );
  printInfo(
    'algorand.account.getAccount(address) retrieves the full AddressWithSigner for an address',
  );
