...
description: 'This example demonstrates how to import and export keys using the KMD `importKey()` and `exportKey()` methods.'
sidebar:
  order: 5
---
...
This example demonstrates how to import and export keys using the KMD `importKey()` and `exportKey()` methods.
Key concepts:
...
```bash
cd examples
npm run example kmd_client/05-key-import-export.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/kmd_client/05-key-import-export.ts)

```typescript title="05-key-import-export.ts"
/**
...
async function main() {
  ...
  try {
    ...
    const keyFound = listResult.addresses.some(
      addr => addr.toString() === importedAddress.toString(),
    );
