...
description: 'This example demonstrates how to use a logic signature (lsig) to authorize transactions.'
sidebar:
  order: 11
---
...
[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/transact/11-logic-sig.ts)

```typescript title="11-logic-sig.ts"
/**
...
async function main() {
  ...
  printInfo(
    `Delegated lsig will authorize transactions FROM: ${shortenAddress(delegator.addr.toString())}`,
  );
  printInfo('');
