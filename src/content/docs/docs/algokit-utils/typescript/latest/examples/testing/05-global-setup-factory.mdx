---
title: 'Vitest Global Setup Factory'
description: 'This example demonstrates how to use createGlobalSetup() to create custom Vitest global setup functions for mock server lifecycle management. It shows:'
sidebar:
  order: 5
---
...
```bash
cd examples
npm run example testing/05-global-setup-factory.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/testing/05-global-setup-factory.ts)

```typescript title="05-global-setup-factory.ts"
/**
...
function main() {
  ...
  printInfo(
    '  function createGlobalSetup(clientType: ClientType): () => Promise<() => Promise<void>>',
  );
  printInfo('');
