...
description: 'This example demonstrates how to manage wallet sessions using KMD handle tokens. Handle tokens are used to unlock wallets and perform operations on them. They have an expiration time and should be released when no longer needed.'
sidebar:
  order: 3
---
...
```bash
cd examples
npm run example kmd_client/03-wallet-sessions.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/kmd_client/03-wallet-sessions.ts)

```typescript title="03-wallet-sessions.ts"
/**
