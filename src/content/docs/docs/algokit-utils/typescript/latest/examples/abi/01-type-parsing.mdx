---
title: 'ABI Type Parsing'
description: 'This example demonstrates how to parse ABI type strings into type objects using ABIType.from(). It shows parsing of:'
sidebar:
  order: 1
---
...
[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/abi/01-type-parsing.ts)

```typescript title="01-type-parsing.ts"
/**
...
function main() {
  ...
  printInfo(
    `ABIType.from('(uint64,address)').equals(ABIType.from('(uint64,address)')): ${tuple1.equals(tuple2)}`,
  );
  printInfo(
    `ABIType.from('(uint64,address)').equals(ABIType.from('(uint64,bool)')): ${tuple1.equals(tuple3)}`,
  );
...
- [AVM Type Encoding](/docs/algokit-utils/typescript/latest/examples/abi/12-avm-types/)
