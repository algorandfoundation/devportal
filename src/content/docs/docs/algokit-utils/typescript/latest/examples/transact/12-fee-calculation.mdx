...
description: 'This example demonstrates how to estimate transaction size and calculate fees using the transact package:'
sidebar:
  order: 12
---
...
```bash
cd examples
npm run example transact/12-fee-calculation.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/transact/12-fee-calculation.ts)

```typescript title="12-fee-calculation.ts"
/**
...
async function main() {
  ...
  const appCallFields: AppCallTransactionFields = {
    ...
    args: [new TextEncoder().encode('arg1'), new TextEncoder().encode('arg2')],
  };
  ...
  printInfo(
    `  (feePerByte * ${paymentEstimatedSize} = ${1n * paymentEstimatedSize} < minFee, so minFee is used)`,
  );
  ...
  printInfo(
    `Extra fee for inner txns: ${extraFeeForInnerTxs} microALGO (${innerTxCount} x ${feePerInnerTx})`,
  );
