---
title: 'App Deployer'
description: 'This example demonstrates the AppDeployer functionality for idempotent application deployment with create, update, and replace strategies:'
sidebar:
  order: 13
---
...
[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/algorand_client/13-app-deployer.ts)

```typescript title="13-app-deployer.ts"
/**
...
async function main() {
  ...
  if (result4.operationPerformed === 'replace' && 'deleteResult' in result4) {
    printInfo(
      `  Delete transaction confirmed: round ${result4.deleteResult.confirmation.confirmedRound}`,
    );
  }
