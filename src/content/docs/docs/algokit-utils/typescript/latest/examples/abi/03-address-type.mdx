---
title: 'ABI Address Type'
description: 'This example demonstrates how to encode and decode Algorand addresses using ABIAddressType:'
sidebar:
  order: 3
---
...
```bash
cd examples
npm run example abi/03-address-type.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/abi/03-address-type.ts)

```typescript title="03-address-type.ts"
/**
...
function main() {
  ...
  const addressObjectMatches = encodedFromAddressObject.every(
    (byte, index) => byte === rawPublicKey[index],
  );
