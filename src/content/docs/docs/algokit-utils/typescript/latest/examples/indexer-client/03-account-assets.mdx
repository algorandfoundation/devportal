...
description: 'This example demonstrates how to query account asset holdings using the IndexerClient lookupAccountAssets() and lookupAccountCreatedAssets() methods.'
sidebar:
  order: 3
---
...
This example demonstrates how to query account asset holdings using the IndexerClient lookupAccountAssets() and lookupAccountCreatedAssets() methods.
...
```bash
cd examples
npm run example indexer_client/03-account-assets.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/indexer_client/03-account-assets.ts)

```typescript title="03-account-assets.ts"
/**
...
async function main() {
  ...
  try {
    ...
    printError(
      `lookupAccountAssets failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  try {
    ...
    printError(
      `lookupAccountCreatedAssets failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  try {
    ...
    printError(
      `Specific asset query failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  try {
    ...
    printError(
      `Specific created asset query failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
