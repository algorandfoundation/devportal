...
description: 'This example demonstrates how to create an ed25519 keypair and sign transactions using the low-level transact package APIs.'
sidebar:
  order: 9
---
...
```bash
cd examples
npm run example transact/09-single-sig.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/transact/09-single-sig.ts)

```typescript title="09-single-sig.ts"
/**
...
async function main() {
  ...
  printInfo(
    `Public key (32 bytes): ${Buffer.from(keypair.publicKey).toString('hex').slice(0, 32)}...`,
  );
  printInfo(
    `Secret key (64 bytes): ${Buffer.from(keypair.secretKey).toString('hex').slice(0, 32)}... (truncated for security)`,
  );
  printInfo('');
