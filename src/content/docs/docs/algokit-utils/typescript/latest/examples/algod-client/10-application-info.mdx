...
description: 'This example demonstrates how to retrieve application information using the AlgodClient method: applicationById()'
sidebar:
  order: 10
---
...
[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/algod_client/10-application-info.ts)

```typescript title="10-application-info.ts"
/**
...
async function main() {
  ...
  try {
    ...
    printError(
      `Failed to deploy or query application: ${error instanceof Error ? error.message : String(error)}`,
    );
