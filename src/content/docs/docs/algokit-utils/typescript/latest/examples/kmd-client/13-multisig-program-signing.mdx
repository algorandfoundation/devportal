---
title: 'Multisig Program Signing (Delegated Multisig Logic Signatures) with KMD'
description: 'This example demonstrates how to sign programs with multisig using the KMD `signMultisigProgram()` method. It shows:'
sidebar:
  order: 13
---
...
This example demonstrates how to sign programs with multisig using the KMD `signMultisigProgram()` method. It shows:
...
```bash
cd examples
npm run example kmd_client/13-multisig-program-signing.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/kmd_client/13-multisig-program-signing.ts)

```typescript title="13-multisig-program-signing.ts"
/**
...
async function main() {
  ...
  try {
    ...
    printInfo(
      `as if signed by the multisig account ${shortenAddress(multisigAddress.toString())}.`,
    );
