...
description: 'This example demonstrates how to access the underlying raw clients through the ClientManager (algorand.client), and how to get typed app clients:'
sidebar:
  order: 14
---
...
[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/algorand_client/14-client-manager.ts)

```typescript title="14-client-manager.ts"
/**
...
async function main() {
  ...
  printInfo(
    `  Genesis Hash: ${Buffer.from(suggestedParams.genesisHash ?? new Uint8Array())
      .toString('base64')
      .slice(0, 20)}...`,
  );
  ...
  printInfo(
    'indexerIfPresent returns undefined if Indexer is not configured (instead of throwing)',
  );
