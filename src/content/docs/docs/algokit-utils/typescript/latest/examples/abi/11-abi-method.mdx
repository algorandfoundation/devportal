---
title: 'ABI Method'
description: 'This example demonstrates how to work with ABI methods:'
sidebar:
  order: 11
---
...
```bash
cd examples
npm run example abi/11-abi-method.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/abi/11-abi-method.ts)

```typescript title="11-abi-method.ts"
/**
 * Example: ABI Method
 *
 * This example demonstrates how to work with ABI methods:
...
function main() {
  ...
  printInfo(
    `  Match: ${Buffer.from(first4Bytes).toString('hex') === Buffer.from(demoMethod.getSelector()).toString('hex')}`,
  );

  // Step 8: Methods with tuple arguments
  printStep(8, 'Methods with Tuple Arguments');

  const tupleArgMethod = ABIMethod.fromSignature(
    'processOrder((uint64,address,uint64),bool)uint64',
  );
