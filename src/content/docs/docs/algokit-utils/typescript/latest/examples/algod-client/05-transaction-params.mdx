...
description: 'This example demonstrates how to get suggested transaction parameters using suggestedParams() and transactionParams() methods. These parameters are essential for constructing valid transactions on the Algorand network.'
sidebar:
  order: 5
---
...
This example demonstrates how to get suggested transaction parameters using suggestedParams() and transactionParams() methods. These parameters are essential for constructing valid transactions on the Algorand network.
...
```bash
cd examples
npm run example algod_client/05-transaction-params.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/algod_client/05-transaction-params.ts)

```typescript title="05-transaction-params.ts"
/**
...
async function main() {
  ...
  try {
    ...
    printInfo(
      `    Original window:     ${(suggestedParams.lastValid - suggestedParams.firstValid).toLocaleString('en-US')} rounds`,
    );
    ...
    printError(
      `Failed to get transaction parameters: ${error instanceof Error ? error.message : String(error)}`,
    );
