---
title: 'Transaction Signing with KMD'
description: 'This example demonstrates how to sign transactions using the KMD `signTransaction()` method. It shows the complete workflow of creating a wallet, generating a key, funding it, signing a transaction, and submitting it to the network.'
sidebar:
  order: 10
---
...
This example demonstrates how to sign transactions using the KMD `signTransaction()` method. It shows the complete workflow of creating a wallet, generating a key, funding it, signing a transaction, and submitting it to the network.
...
```bash
cd examples
npm run example kmd_client/10-transaction-signing.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/kmd_client/10-transaction-signing.ts)

```typescript title="10-transaction-signing.ts"
/**
...
async function main() {
  ...
  try {
    ...
    const expectedBalance =
      fundAmountMicroAlgos - paymentAmount - (transaction.fee ?? suggestedParams.minFee);
...
- [KMD Version Information](/docs/algokit-utils/typescript/latest/examples/kmd-client/01-version/)
