---
title: 'Light Block Header Proof'
description: 'This example demonstrates how to get light block header proofs using:'
sidebar:
  order: 16
---
...
```bash
cd examples
npm run example algod_client/16-lightblock-proof.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/algod_client/16-lightblock-proof.ts)

```typescript title="16-lightblock-proof.ts"
/**
...
async function main() {
  ...
  printInfo(
    '  - Light block header proofs are only available for rounds within a state proof interval',
  );
...
function displayLightBlockHeaderProof(proof: LightBlockHeaderProof, round: bigint): void {
  ...
  printInfo(
    `               Number of edges from leaf to root (tree can hold ${Math.pow(2, proof.treedepth)} headers)`,
  );
  printInfo('');
  ...
  printInfo(
    `    Interval Range: rounds ${intervalStart.toLocaleString('en-US')} to ${intervalEnd.toLocaleString('en-US')}`,
  );
  printInfo('');
}
