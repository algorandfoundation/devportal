---
title: 'Transaction Composer (Atomic Transaction Groups)'
description: 'This example demonstrates how to build atomic transaction groups using the transaction composer:'
sidebar:
  order: 10
---
...
```bash
cd examples
npm run example algorand_client/10-transaction-composer.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/algorand_client/10-transaction-composer.ts)

```typescript title="10-transaction-composer.ts"
/**
...
async function main() {
  ...
  for (let i = 0; i < groupResult.txnResults.length; i++) {
    ...
    printInfo(
      `  [${i}]: Confirmed round: ${txnResult.txnResult.confirmedRound ?? 'N/A (simulated)'}`,
    );
  }
  ...
  const allSameGroup = transactions.every(
    txn =>
      txn.group &&
      firstGroupId &&
      Buffer.from(txn.group).toString('base64') === Buffer.from(firstGroupId).toString('base64'),
  );
  ...
  printInfo(`\nSigner breakdown:`);
  printInfo(
    `  [0] Asset Opt-In: Sender ${shortenAddress(multiSignerResult.transactions[0].sender.toString())} (Bob)`,
  );
  printInfo(
    `  [1] Asset Transfer: Sender ${shortenAddress(multiSignerResult.transactions[1].sender.toString())} (Alice)`,
  );
  printInfo(
    `  [2] Payment: Sender ${shortenAddress(multiSignerResult.transactions[2].sender.toString())} (Charlie)`,
  );
