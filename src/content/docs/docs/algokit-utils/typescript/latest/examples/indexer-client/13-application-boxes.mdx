---
title: 'Application Boxes Lookup'
description: 'This example demonstrates how to query application boxes using the IndexerClient searchForApplicationBoxes() and lookupApplicationBoxByIdAndName() methods.'
sidebar:
  order: 13
---
...
[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/indexer_client/13-application-boxes.ts)

```typescript title="13-application-boxes.ts"
/**
...
async function main() {
  ...
  try {
    ...
    printError(
      `searchForApplicationBoxes failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  try {
    ...
    printError(
      `lookupApplicationBoxByIdAndName failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  try {
    ...
    printError(
      `Box name encoding demo failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
