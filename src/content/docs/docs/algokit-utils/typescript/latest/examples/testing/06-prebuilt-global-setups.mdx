---
title: 'Pre-built Global Setups'
description: 'This example demonstrates the pre-built global setup functions available for common client types and how to use them in Vitest configuration. It shows:'
sidebar:
  order: 6
---
...
```bash
cd examples
npm run example testing/06-prebuilt-global-setups.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/testing/06-prebuilt-global-setups.ts)

```typescript title="06-prebuilt-global-setups.ts"
/**
...
function main() {
  ...
  printInfo(
    '  import { algodGlobalSetup, indexerGlobalSetup } from "@algorandfoundation/algokit-utils/testing"',
  );
  printInfo('');
