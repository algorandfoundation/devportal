---
title: 'Sync Round Management'
description: 'This example demonstrates how to manage the sync round using:'
sidebar:
  order: 19
---
...
```bash
cd examples
npm run example algod_client/19-sync-round.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/algod_client/19-sync-round.ts)

```typescript title="19-sync-round.ts"
/**
...
async function main() {
  ...
  try {
    ...
    printSuccess(
      `Block ${currentRound} is accessible (timestamp: ${block.block.header.timestamp})`,
    );
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
