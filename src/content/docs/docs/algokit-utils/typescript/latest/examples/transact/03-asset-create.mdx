---
title: 'Asset Create'
description: 'This example demonstrates how to create a new Algorand Standard Asset (ASA) using the transact package. It shows the low-level transaction construction pattern with:'
sidebar:
  order: 3
---
...
[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/transact/03-asset-create.ts)

```typescript title="03-asset-create.ts"
/**
...
async function main() {
  ...
  printInfo(
    `Creator balance: ${creatorBalance} (${Number(creatorBalance) / Math.pow(10, assetDecimals)} ${assetUnitName})`,
  );
