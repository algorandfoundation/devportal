---
title: 'Key Listing and Deletion'
description: 'This example demonstrates how to list all keys in a wallet and delete specific keys using the KMD `listKeysInWallet()` and `deleteKey()` methods.'
sidebar:
  order: 6
---
...
This example demonstrates how to list all keys in a wallet and delete specific keys using the KMD `listKeysInWallet()` and `deleteKey()` methods.
...
```bash
cd examples
npm run example kmd_client/06-key-listing-deletion.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/kmd_client/06-key-listing-deletion.ts)

```typescript title="06-key-listing-deletion.ts"
/**
