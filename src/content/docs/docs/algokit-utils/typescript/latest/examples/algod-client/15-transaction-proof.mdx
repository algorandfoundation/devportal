...
description: 'This example demonstrates how to get transaction proofs using:'
sidebar:
  order: 15
---
...
```bash
cd examples
npm run example algod_client/15-transaction-proof.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/algod_client/15-transaction-proof.ts)

```typescript title="15-transaction-proof.ts"
/**
...
async function main() {
  ...
  try {
    ...
    printInfo(
      `     A tree with depth ${proof.treedepth} can hold up to ${Math.pow(2, proof.treedepth)} transactions.`,
    );
    printInfo('');
  ...
  printInfo(
    '   - Verify a transaction is included in the blockchain without downloading all blocks',
  );
