---
title: 'Mock Server Connection'
description: 'This example demonstrates how to use getMockServer() to connect to a mock server and understand the MockServer interface. It shows:'
sidebar:
  order: 4
---
...
```bash
cd examples
npm run example testing/04-mock-server-connection.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/testing/04-mock-server-connection.ts)

```typescript title="04-mock-server-connection.ts"
/**
...
async function main() {
  ...
  try {
    ...
    printInfo(
      '  const client = new AlgodClient({ baseUrl: server.baseUrl, token: DEFAULT_TOKEN })',
    );
  } catch (error) {
