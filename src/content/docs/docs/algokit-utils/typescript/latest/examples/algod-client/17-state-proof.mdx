---
title: 'State Proof'
description: 'This example demonstrates how to get state proofs using:'
sidebar:
  order: 17
---
...
```bash
cd examples
npm run example algod_client/17-state-proof.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/algod_client/17-state-proof.ts)

```typescript title="17-state-proof.ts"
/**
 * Example: State Proof
 *
 * This example demonstrates how to get state proofs using:
...
async function main() {
  ...
  printInfo(
    `Example: If state proof exists for round ${exampleProofRound.toLocaleString('en-US')}:`,
  );
  printInfo(
    `  It would attest to rounds ${intervalStart.toLocaleString('en-US')} to ${intervalEnd.toLocaleString('en-US')}`,
  );
  printInfo('');
  ...
  try {
    ...
    if (
      errorMessage.includes('state proof') ||
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    printInfo(
      `    Error (expected): ${errorMessage.substring(0, 80)}${errorMessage.length > 80 ? '...' : ''}`,
    );
  }
  printInfo('');
...
function displayStateProof(proof: StateProof): void {
  ...
  printInfo(
    `    blockHeadersCommitment: ${Buffer.from(proof.message.blockHeadersCommitment).toString('hex').substring(0, 64)}...`,
  );
  ...
  printInfo(
    `    votersCommitment: ${Buffer.from(proof.message.votersCommitment).toString('hex').substring(0, 64)}...`,
  );
  ...
  printInfo('  Coverage:');
  printInfo(
    `    This state proof attests to rounds ${proof.message.firstAttestedRound.toLocaleString('en-US')} to ${proof.message.lastAttestedRound.toLocaleString('en-US')}`,
  );
