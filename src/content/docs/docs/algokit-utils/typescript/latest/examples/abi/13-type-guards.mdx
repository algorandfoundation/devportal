---
title: 'Type Guards'
description: 'This example demonstrates how to use type guard functions to check argument and type categories in the ABI system:'
sidebar:
  order: 13
---
...
```bash
cd examples
npm run example abi/13-type-guards.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/abi/13-type-guards.ts)

```typescript title="13-type-guards.ts"
/**
 * Example: Type Guards
 *
...
function main() {
  ...
  printInfo('Functions:');
  printInfo(
    '  argTypeIsTransaction(type) - Returns true for txn, pay, keyreg, acfg, axfer, afrz, appl',
  );
