---
title: 'Atomic Swap'
description: 'This example demonstrates how to perform an atomic swap of ALGO for ASA between two parties. In an atomic swap:'
sidebar:
  order: 8
---
...
[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/transact/08-atomic-swap.ts)

```typescript title="08-atomic-swap.ts"
/**
 * Example: Atomic Swap
 *
...
async function main() {
  ...
  printInfo(
    `  - ${assetUnitName}: ${partyAAssetBalance} (sent ${swapAssetAmount}, remaining: ${assetTotal - swapAssetAmount})`,
  );
  ...
  if (partyBAssetBalance !== swapAssetAmount) {
    throw new Error(
      `Party B ASA balance mismatch: expected ${swapAssetAmount}, got ${partyBAssetBalance}`,
    );
  }
  ...
  if (partyAAssetBalance !== expectedPartyAAsaBalance) {
    throw new Error(
      `Party A ASA balance mismatch: expected ${expectedPartyAAsaBalance}, got ${partyAAssetBalance}`,
    );
  }
  ...
  printSuccess(
    `Party B sent ${formatAlgo(swapAlgoAmount)} (plus ${formatAlgo(partyBAlgoDecrease - swapAlgoAmount)} in fees)`,
  );
