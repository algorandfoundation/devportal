---
title: 'Atomic Transaction Group'
description: 'This example demonstrates how to group multiple transactions atomically. All transactions in a group either succeed together or fail together. It shows:'
sidebar:
  order: 7
---
...
```bash
cd examples
npm run example transact/07-atomic-group.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/transact/07-atomic-group.ts)

```typescript title="07-atomic-group.ts"
/**
...
async function main() {
  ...
  printInfo(
    `Total fees: ${(tx1WithFee.fee ?? 0n) + (tx2WithFee.fee ?? 0n) + (tx3WithFee.fee ?? 0n)} microALGO`,
  );
  ...
  printInfo(
    `Group ID (base64): ${groupId ? Buffer.from(groupId).toString('base64') : 'undefined'}`,
  );
  ...
  printInfo(
    `Submitting ${groupedTransactions.length} grouped transactions as a single atomic unit`,
  );
  ...
  printInfo(
    `Receiver 1 balance: ${formatAlgo(receiver1Balance.microAlgo)} (expected: ${formatAlgo(amounts[0])})`,
  );
  printInfo(
    `Receiver 2 balance: ${formatAlgo(receiver2Balance.microAlgo)} (expected: ${formatAlgo(amounts[1])})`,
  );
  printInfo(
    `Receiver 3 balance: ${formatAlgo(receiver3Balance.microAlgo)} (expected: ${formatAlgo(amounts[2])})`,
  );
  ...
  printInfo(
    `Sender balance change: ${formatAlgo(senderBalance.microAlgo - senderFinalBalance.microAlgo)}`,
  );
