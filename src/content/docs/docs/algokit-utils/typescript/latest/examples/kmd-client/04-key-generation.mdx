...
description: "This example demonstrates how to generate new keys in a wallet using the KMD `generateKey()` method. Keys are generated deterministically from the wallet's master derivation key, which means they can be recovered if you have the wallet's mnemonic or master derivation key."
sidebar:
  order: 4
---
...
This example demonstrates how to generate new keys in a wallet using the KMD `generateKey()` method. Keys are generated deterministically from the wallet's master derivation key, which means they can be recovered if you have the wallet's mnemonic or master derivation key.
...
```bash
cd examples
npm run example kmd_client/04-key-generation.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-utils-ts/blob/main/examples/kmd_client/04-key-generation.ts)

```typescript title="04-key-generation.ts"
/**
