---
editUrl: false
next: false
prev: false
title: "arc56MethodToABIMethod"
---

> **arc56MethodToABIMethod**(`method`, `appSpec`): [`ABIMethod`](/docs/algokit-utils/typescript/latest/api/subpaths/abi/classes/abimethod/)

Defined in: [packages/abi/src/abi-method.ts:205](https://github.com/larkiny/algokit-utils-ts/blob/main/packages/abi/src/abi-method.ts#L205)

## Parameters

### method

[`Arc56Method`](/docs/algokit-utils/typescript/latest/api/subpaths/abi/type-aliases/arc56method/)

### appSpec

[`Arc56Contract`](/docs/algokit-utils/typescript/latest/api/subpaths/abi/type-aliases/arc56contract/)

## Returns

[`ABIMethod`](/docs/algokit-utils/typescript/latest/api/subpaths/abi/classes/abimethod/)
