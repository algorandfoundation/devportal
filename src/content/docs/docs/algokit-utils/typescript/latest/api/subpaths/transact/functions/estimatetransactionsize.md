---
editUrl: false
next: false
prev: false
title: "estimateTransactionSize"
---

> **estimateTransactionSize**(`transaction`): `bigint`

Defined in: [packages/transact/src/transactions/transaction.ts:515](https://github.com/larkiny/algokit-utils-ts/blob/main/packages/transact/src/transactions/transaction.ts#L515)

Return the size of the transaction in bytes as if it was already signed and encoded.
This is useful for estimating the fee for the transaction.

## Parameters

### transaction

[`Transaction`](/docs/algokit-utils/typescript/latest/api/subpaths/transact/classes/transaction/)

## Returns

`bigint`
