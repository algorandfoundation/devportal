...
The entry point to the vast majority of functionality in AlgoKit Utils is now available via a single entry-point, the [`AlgorandClient` class](/docs/algokit-utils/typescript/latest/guides/concepts/core/algorand-client/).
...
- The `resolveBy` functionality has disappeared in favour of [much simpler entrypoints within `algorand.client`](/docs/algokit-utils/typescript/latest/guides/concepts/building/app-client/#appclient)
