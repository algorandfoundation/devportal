...
- `testAccountFunding?: AlgoAmount` - The [amount](/docs/algokit-utils/typescript/latest/concepts/core/amount/) of funds to allocate to the default testing account, if not specified then it will get `10` ALGO
...
- `algorand: AlgorandClient` - An [`AlgorandClient`](/docs/algokit-utils/typescript/latest/concepts/core/algorand-client/) instance
...
The key when generating a test account is getting hold of a [dispenser](/docs/algokit-utils/typescript/latest/concepts/building/transfer/#dispenser) and then [ensuring the test account is funded](/docs/algokit-utils/typescript/latest/concepts/building/transfer/#ensurefunded).

To make it easier to quickly get a test account the testing capability provides the following mechanisms:

- `algotesting.getTestAccount(testAccountParams, algod, kmd?)` - Generates a random new account, logs the mnemonic of the account (unless suppressed), funds it from the [dispenser](/docs/algokit-utils/typescript/latest/concepts/building/transfer/#dispenser)
