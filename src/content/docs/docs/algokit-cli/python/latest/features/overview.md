...
- [Project](/docs/algokit-cli/python/latest/features/project/) - Manage an AlgoKit project workspace on your file system
- [Tasks](/docs/algokit-cli/python/latest/features/tasks/) - Perform a variety of useful operations on the Algorand blockchain
...
- `--skip-version-check` Skips updated AlgoKit version checking and prompting for that execution, this can also be disabled [permanently on a given machine](/docs/algokit-cli/python/latest/cli/#version-prompt) with `algokit config version-prompt disable`.

See also the [AlgoKit CLI Reference](/docs/algokit-cli/python/latest/cli/), which details every command, sub-command and option.
