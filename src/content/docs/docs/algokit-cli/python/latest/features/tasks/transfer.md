...
> Note: If you use a wallet address for the `sender` argument, you'll be asked for the mnemonic phrase. To use a wallet alias instead, see the [wallet aliasing](/docs/algokit-cli/python/latest/features/tasks/wallet/) task. For wallet aliases, the sender must have a stored `private key`, but the receiver doesn't need one. This is because the sender signs and sends the transfer transaction, while the receiver reference only needs a valid Algorand address.
...
For in-depth details, visit the [transfer section](/docs/algokit-cli/python/latest/cli/#transfer) in the AlgoKit CLI reference documentation.
