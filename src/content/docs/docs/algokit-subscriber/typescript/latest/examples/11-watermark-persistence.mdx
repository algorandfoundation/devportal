...
description: 'This example demonstrates file-backed watermark persistence across polls.'
sidebar:
  order: 11
---
...
[View source on GitHub](https://github.com/algorandfoundation/algokit-subscriber-ts/blob/main/examples/subscriber/11-watermark-persistence.ts)

```typescript title="11-watermark-persistence.ts"
/**
...
async function main() {
  ...
  if (savedWatermark <= startWatermark) {
    throw new Error(
      `Watermark should have advanced past ${startWatermark}, but is ${savedWatermark}`,
    );
  }
  ...
  const poll2Notes = poll2Txns.map(txn =>
    txn.note ? Buffer.from(txn.note).toString('utf-8') : '',
  );
  ...
  if (finalWatermark <= savedWatermark) {
    throw new Error(
      `Final watermark should have advanced past ${savedWatermark}, but is ${finalWatermark}`,
    );
  }
