---
title: 'App Call Subscription'
description: 'This example demonstrates application call subscription.'
sidebar:
  order: 5
---
...
[View source on GitHub](https://github.com/algorandfoundation/algokit-subscriber-ts/blob/main/examples/subscriber/05-app-call.ts)

```typescript title="05-app-call.ts"
/**
...
async function main() {
  ...
  const createTxns = await testFilter(
    ...
    txn => {
      ...
      printInfo(
        `  Created app: ${txn.createdAppId} | onComplete: ${appTxn?.onCompletion} | txn: ${txn.id}`,
      );
    },
  );
  ...
  const methodTxns = await testFilter(
    ...
    txn => {
      ...
      const selectorHex =
        appArgs && appArgs.length > 0 ? Buffer.from(appArgs[0].slice(0, 4)).toString('hex') : 'N/A';
      printInfo(
  ...
  printInfo(
    `appCallArgumentsMatch filter: ${argMatchTxns.length} matched (emitSwapped by selector)`,
  );
