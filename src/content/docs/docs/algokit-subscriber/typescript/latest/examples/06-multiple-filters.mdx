...
description: 'This example demonstrates multiple named filters with deduplication.'
sidebar:
  order: 6
---
...
npx tsx 06-multiple-filters.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-subscriber-ts/blob/main/examples/subscriber/06-multiple-filters.ts)

```typescript title="06-multiple-filters.ts"
/**
...
async function main() {
  ...
  const txnSpecs = [
    ...
    },
    {
      ...
      amount: algo(1),
      note: 'multi-02',
      desc: 'sender->sender 1A (from-sender)',
    },
    {
      ...
      amount: algo(4),
      note: 'multi-03',
      desc: 'receiver->sender 4A (large-txns)',
    },
    {
      sender: sender.addr,
      receiver: receiver.addr,
      amount: algo(1),
      note: 'multi-04',
      desc: 'sender->receiver 1A (from-sender + to-receiver)',
    },
    {
      sender: receiver.addr,
      receiver: sender.addr,
      ...
      note: 'multi-05',
      desc: 'receiver->sender 0.5A (none)',
    },
  ];
  ...
  if (txn1Matches.length !== 1) {
    throw new Error(
      `Dedup failed: txn "${txn1Note}" appears ${txn1Matches.length} times, expected 1`,
    );
  }
  printInfo(
    `Txn "multi-01": appears ${txn1Matches.length} time (matched ${txn1Matches[0].filtersMatched?.length} filters)`,
  );
  ...
  if (
    !txn1Filters.includes('from-sender') ||
    !txn1Filters.includes('to-receiver') ||
    !txn1Filters.includes('large-txns')
  ) {
  ...
  if (
    txn4Filters.length !== 2 ||
    !txn4Filters.includes('from-sender') ||
    !txn4Filters.includes('to-receiver')
  ) {
    throw new Error(
      `multi-04 expected [from-sender, to-receiver], got: [${txn4Filters.join(', ')}]`,
    );
  }
  ...
  if (txn5Match) {
    throw new Error(
      `multi-05 should not match any filter but appeared with: [${txn5Match.filtersMatched?.join(', ')}]`,
    );
  }
  ...
  console.log('  ├────────────┼─────────────────────────┼───────────────────────────────────────┤');
  console.log(
    `  │ ${'multi-05'.padEnd(10)} │ ${'500000 microALGO'.padEnd(23)} │ ${'(no match — excluded)'.padEnd(37)} │`,
  );
  console.log('  └────────────┴─────────────────────────┴───────────────────────────────────────┘');
  console.log();
