...
description: 'This example demonstrates custom filter predicates with multi-condition logic.'
sidebar:
  order: 13
---
...
npx tsx 13-custom-filters.ts
```

## Code

[View source on GitHub](https://github.com/algorandfoundation/algokit-subscriber-ts/blob/main/examples/subscriber/13-custom-filters.ts)

```typescript title="13-custom-filters.ts"
/**
...
async function main() {
  ...
  for (const [i, p] of payments.entries()) {
    ...
    const reasons = [];
    if (!amountOk)
      reasons.push(`amount ${formatMicroAlgo(amount)} < ${formatMicroAlgo(THRESHOLD)}`);
  ...
  printStep(
    5,
    'Composition: sender=sender account (standard) + customFilter (amount >= 2 ALGO AND note contains "transfer")',
  );
  ...
  await testFilter('inspect', {
    customFilter: (txn: SubscribedTransaction) => {
      ...
      if (inspectedFields.length === 0) {
        const fields = Object.keys(txn).filter(
          k => txn[k as keyof SubscribedTransaction] !== undefined,
        );
  ...
  printStep(7, 'Summary');
  printInfo(
    `Custom filter only: ${customOnlyTxns.length} matched (multi-condition: amount + note + sender allowlist)`,
  );
  printInfo(
    `Composed filter: ${composedTxns.length} matched (standard sender + custom amount/note)`,
  );
  printInfo(`Key takeaway: customFilter is AND-composed with standard filter fields`);
  printInfo(
    `Key takeaway: customFilter receives the full SubscribedTransaction with all decoded fields`,
  );
