...
**File permissions.** Private key files are written with mode `0600`. Public key files are written
with mode `0666`.
...
sender. When the key does match the sender, `AuthAddr` is left empty.
...
been rekeyed to a multisig account. It writes the fields only; it adds no signature.
...
| `--dilution` | uint64 | No       | Key dilution for two-level keys. Defaults to the square root of the validity window. |
...
| Name               | Type   | Required | Description                                                                                                              |
| ------------------ | ------ | -------- | ------------------------------------------------------------------------------------------------------------------------ |
| `--firstvalid`     | uint64 | **Yes**  | First round the transaction may be committed.                                                                            |
...
| `--fee`            | uint64 | No       | Transaction fee in microAlgos. Default `1000`.                                                                           |
...
| `--outputFile, -o` | string | No       | Path to write the transaction, or `-` for standard output.                                                               |
...
All errors are written to standard error with exit code `1`.
...
| `Cannot recover key seed from mnemonic`                         | Mnemonic fails checksum or word validation              |
| `pq signature scheme not supported: "<scheme>"`                 | `--scheme` names a scheme other than `falcon-1024`/`f1` |
| `pq key file is malformed`                                      | PQ key file fails to decode or has wrong key sizes      |
| `transaction already has a signature`                           | `pq sign` on a signed transaction without `--overwrite` |
| `<file> looks like TEAL source; compile it first`               | `pq sign-program` given a `.teal` filename              |
| `program is not compiled bytecode; compile it first`            | `pq sign-program` given all-printable-ASCII input       |
