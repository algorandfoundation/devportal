...
The languages named above are not recommended for new development. Write new smart contracts in [Algorand Python](/concepts/smart-contracts/languages/python) or [Algorand TypeScript](/concepts/smart-contracts/languages/typescript), where automatic ABI validation is applied by default in the fixed Puya versions listed above.
