...
maximum security, either use a catchpoint from your own archiver node or synchronize from genesis.

:::caution
Do not use fast-catchup on archiver or repeater nodes. If you accidentally do so, you must reset your
...
2. Use the catchup command with your network's current catchpoint from the catchpoint URL:
...
- Your node is not configured as archiver or repeater
...
If you accidentally used fast-catchup on an archiver node:
...
:::note
Nodely's telemetry service is designed for MainNet nodes including validators, API Providers, and P2P
nodes/repeaters. Do not use it for the Repeater/Archiver Running Program.
:::
