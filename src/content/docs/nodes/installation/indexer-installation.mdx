...
Indexer populates this database by connecting to an Algorand archiver node and processing all ledger
...
To populate the PostgreSQL database, provide the Algorand Archiver node connection details using
either:
...
2. Ensure the node is in archiver mode
