...
The Algorand network is comprised of two distinct types of nodes, **repeater nodes**, and **non-repeater nodes**. Repeater nodes are primarily used for communication routing to a set of connected non-repeater nodes. Repeater nodes communicate with other repeater nodes and route blocks to all connected non-repeater nodes. Non-repeater nodes only connect to repeater nodes and can also participate in consensus. Non-repeater nodes may connect to several repeater nodes but never connect to another non-repeater node.

In addition to the two node types, nodes can be configured to be an [**Archiver**](#archiver-mode). Archiver nodes store the entire ledger, as opposed to the last 1000 blocks for non-archiver nodes. Repeater nodes are necessarily archiver nodes as well. Non-repeater archiver nodes are often used to feed an [**indexer**](/nodes/installation/indexer-installation) that allows more advanced queries on the history of the blockchain.
...
All node types use the same install procedure. To setup a node for a specific type, requires a few configuration parameter changes or operations as described below. The default install will set the node up as a non-repeater non-archiver non-participation mode, also known as an **API provider** node.
...
Non-validator nodes maintain a current copy of the blockchain but do not participate in
...
- I want to participate in consensus and help secure the Algorand network.
  - Validator node: Non-repeater, non-archiver participation node
...
- I want to send transactions and read current state of smart contracts/applications:
  - Non-repeater, non-archiver non-participation node
...
- I want full access to historical data (blocks, transactions) with advanced querying:
  - Non-repeater, archiver non-participation node, together with an [**Indexer**](/nodes/installation/indexer-installation).
- I want to get state proofs for any block:
  - Non-repeater, archiver non-participation node

## Validator Node

How to install a node is described [here](/nodes/installation/manual-installation). Classifying a node as a validator node is not a configuration parameter but a dynamic operation where the node is hosting participation keys for one or more online accounts. This process is described in [Participate in Consensus](/concepts/protocol/overview). Technically both non-repeater and repeater nodes can participate in consensus, but Algorand recommends _only_ non-repeater nodes participate in consensus.
...
A repeater node uses the same software install as a non-repeater node and only requires setting a few additional configuration parameters.
