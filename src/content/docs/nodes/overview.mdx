...
Running an Algorand node is essential for participating in and maintaining the blockchain network. Whether you choose to run a repeater node for network communication, a participation node for consensus voting, or an archiver node for storing the complete ledger, your node plays a vital role in the network's security and decentralization. This documentation provides a comprehensive guide to understanding and running Algorand nodes.
...
- **Non-Repeater Nodes:** Connect exclusively to repeater nodes and can participate in consensus. While
  non-repeater nodes may establish connections with multiple repeater nodes, they do not connect directly
  to other non-repeater nodes.
...
- **Non-Archiver Nodes**: Retain only the last 1000 blocks.
