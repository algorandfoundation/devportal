...
To get ready to set up an Algorand validator node, there are a few prerequisites to get sorted.
This section covers preparing your machine with the foundation on which the node will be run.
Running archiver and repeater nodes requires additional configuration and considerations that are not
...
### Ensure that Validator Nodes are Working Properly

Monitor your validator node to ensure high performance and consistent access to your registered
...
- The validator node is sending votes and proposing blocks at the expected frequency.
...
##### Dont add many accounts to a single validator node
...
Running a validator node on a machine with less than 16GB of RAM without configuring GOMEMLIMIT
