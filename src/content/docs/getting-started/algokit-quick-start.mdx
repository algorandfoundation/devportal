...
This extension exposes the following settings:
<Steps>
1. `algorandPython.languageServer.enable`: Enables the Algorand Python language server. By default the language server is disabled.
2. `algorandPython.languageServer.logLevel`: Configures the verbosity of the messages Algorand Python language server displays in the output window.
3. `algorandPython.languageServer.debounceInterval`: Configures the delay between receiving a code change and refreshing related diagnostics.
</Steps>
