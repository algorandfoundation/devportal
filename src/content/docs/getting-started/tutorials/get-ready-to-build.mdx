...
<Steps>
  ...
  3. Enable the Algorand TypeScript
  language server (`algorandTypeScript.languageServer.enable`) for your workspace (not required if using a new AlgoKit project) 
...
<Steps>
  ...
  default, the language server is disabled. 
  2. `algorandTypeScript.languageServer.logLevel`:
...
<Steps>
  ...
  2. Install the Algorand Python language extension 
...
<Steps>
  ...
  the language server is disabled. 
  2. `algorandPython.languageServer.logLevel`: Configures the
  verbosity of the messages Algorand Python language server displays in the output window. 
  3. `algorandPython.languageServer.debounceInterval`: Configures the delay between receiving a code
...
If you already have Python 3.12+ installed, you may prefer to use `pipx install algokit` as explained within the pipx on any OS section, so you can control the Python version used.
:::
...
When building smart contracts, it is recommended to build and test on a local Algorand blockchain running on your computer first. Deploying and calling smart contracts on MainNet (the live public blockchain for production) costs real money. Deploying on TestNet (a replica of MainNet for testing) can slow down the iteration cycle of early projects. Therefore, it is recommended to first test on the local blockchain. Once everything works, move to TestNet for final testing, and then deploy to MainNet to launch your application.
...
<Tabs syncKey="lang">
    <TabItem label="TypeScript" icon="seti:typescript">
        For this guide, we will use the TypeScript smart contract starter template.
            ...
            <Steps>
                ...
                After entering the name, press Enter to continue.<Figure src={algokitInitTs2} alt="AlgoKit Init: Specifying folder name" />
                ...
                5. Next, you can select the template preset. The **Starter** preset is for quickly implementing and testing certain features, whereas the **Production** preset is for production-ready applications. For this guide, let's select **Starter**.<Figure src={algokitInitTs4} alt="AlgoKit Init: Select template preset" />

                6. Then choose whether you want to run the `algokit project bootstrap` command to install the necessary dependencies.
    ...
    <TabItem label="Python" icon="seti:python">
        ...
        <Steps>
            ...
            5. Next, you can select the template preset. The **Starter** preset is for quickly implementing and testing certain features, whereas the **Production** preset is for production-ready applications. For this guide, let's select **Starter**.<Figure src={algokitInitPy5} alt="AlgoKit Init: Select template preset" />

            6. You'll be prompted to choose what language you want to use for contract deployment. After writing your smart contract, you need to deploy and call the contract either from your frontend or backend. Since your deployment and interaction scripts are written in Python, select Python for this guide.
               <Figure src={algokitInitPy6} alt="AlgoKit Init: Selecting language for contract deployment script" />

            7. Then choose whether you want to run the `algokit project bootstrap` command to install the necessary dependencies.
            Say yes by pressing "y". This process can take a few minutes.
               <Figure src={algokitInitPy7} alt="AlgoKit Init: Running the bootstrap command" />
            ...
            8. Finally, AlgoKit will ask you if you want to initialize a Git repository. Let's say yes by pressing "y" so that we can easily push our AlgoKit project to GitHub.
               <Figure src={algokitInitPy8} alt="AlgoKit Init: Initialize Git repository" />
...
## Before Moving On
   ...
   If not, something went sideways—fix it now before continuing.
