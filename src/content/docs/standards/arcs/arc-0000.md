...
Parties involved in the process are you, the champion or _ARC author_, the [_ARC editors_](#arc-editors), the <a href="https://github.com/orgs/algorand/people">_Algorand Core Developers_</a>, and the <a href="https://github.com/orgs/algorandfoundation/people">_Algorand Foundation Team_</a>.
...
_In short, the role of a champion is to write the ARC using the style and format described below, shepherd the discussions in the appropriate forums, build community consensus around the idea, and gather projects with similar needs who will implement it._
...
> _ARC Authors are notified of any algorithmic change to the status of their ARC_
...
` arc:` _ARC number_ (It is determined by the ARC editor)

` title:` _The ARC title is a few words, not a complete sentence_
...
` * last-call-deadline:` _Date review period ends_

` type:` _Standards Track, Meta, or Informational_

` * category:` _Core, Networking, Interface, or ARC_ (Only needed for Standards Track ARCs)
...
> ` * resolution:` _A url pointing to the resolution of this ARC_
  ...
  "methods": [
    {
      "name": "arc0_method1",
      "desc": "Method 1",
      "args": [{ "type": "uint64", "name": "Number", "desc": "A number" }],
      "returns": { "type": "void[]" }
    },
    {
      "name": "arc0_method2",
...
If for some reason (CCO, RFC ...), you need to refer on something outside of the repository, you _MUST_ you the following syntax
...
- Create a living discussion in the Issues section of this repository
  ...
  > The issue will be closed when the ARC reaches the status _Final_ or _Withdrawn_
