...
The specification for interfaces is defined by [ARC-4](../arc-0004).
...
Selectors in the interface include selectors for methods, selectors for events as defined by [ARC-28](../arc-0028), and selectors for potential future kinds of interface components.
...
The `supportsInterface` method must be `readonly` as specified by [ARC-22](../arc-0022).

The implementing contract must have a `supportsInterface` method that returns:

* `true` when `interfaceID` is `0x4e22a3ba` (the selector for [ARC-73](../arc-0073), this interface)
