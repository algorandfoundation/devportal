...
:::note
This guide covers bridging between AVM and EVM-compatible chains only. It currently doesnt support bridging between any other chains.
:::
...
:::note The token is in burning mode since the token already existed in Algorand. If your token
already exists, then this should be `locking` mode. ::: **Save the addresses you get in your
`deployment.json` file for peer configuration!**
...
Incase you dont have FOLKS token to pay fees, you can update `feePaymentToken` to use native ALGO for fee payment
...
Incase you dont have FOLKS token to pay fees, you can update `feePaymentToken` to use native ETH token for fee payment
...
|         ├─> Get funds from [Lora](https://www.lora.algokit.io/testnet/funds)
